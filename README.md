...
| `log_file`      | append the (secret-redacted) log to this file as well as stdout; empty → stdout only                                                                                                                                                                    |
| `verbose`       | stream the agent's live output into the log (same as `--verbose`); handy for watching what a job is doing                                                                                                                                                |
| `allow_web`     | append `WebFetch` to the agent's allowed tools (off by default)                                                                                                                                                                                         |
| `ack_start`     | post the "picked this up" ack when a job starts (default true; set false for quieter threads)                                                                                                                                                            |
| `ack_finish`    | post the "opened PR / pushed commits" ack when a job finishes (default true). The PR body / report comment still carries the substance either way                                                                                                        |
| `impersonate`   | commit under the owner's identity instead of the bot's (default false). Uses `commit_name` / `commit_email` when set, else the owner's login + GitHub noreply address                                                                                    |
| `work_root`     | if set, each issue/PR gets a **persistent** checkout dir under here (instead of a throwaway temp dir), and a claude runner resumes the **same Claude session** per issue/PR across follow-ups via `--session-id` / `--resume`. Empty → temp dirs, no session reuse |
