...
@mentioned it (GitHub search, all public repos). Only issues *you* authored
...
- A **dedicated bot GitHub account** with a PAT. Two modes:
  - **Collaborator mode** (private repos, direct pushes): invite the bot as
    ...
    *accept* invites (GitHub only offers read-only invitation permission) —
...
Only events *after* the first start are picked up (no backfill), except
already-open issues assigned to or mentioning the bot, which are picked up on
the first poll.

### Run as a service (macOS)

`~/Library/LaunchAgents/info.stamat.gh-overseer.plist`:
...
files, and commits works:

```json
"runner": ["mini", "--yolo", "-t", "{prompt}"]
```

(e.g. [mini-swe-agent](https://github.com/SWE-agent/mini-swe-agent),
...
- **Owner authorship is the security boundary.** Jobs start only from issues
  *authored by you* and assigned to / mentioning the bot; follow-ups only
  from *your* comments/reviews on the bot's threads. Everyone else is
